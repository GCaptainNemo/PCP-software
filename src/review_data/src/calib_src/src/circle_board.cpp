...
public:
    ...
    Mat nextImage(std::string &img_address)
    {
        Mat result;
        if( inputCapture.isOpened() )
        {
            ...
            img_address = imageList[atImageList++];
            result = imread(img_address, IMREAD_COLOR);
        }
        return result;
    }
...
int main(int argc, char* argv[])
{
	...
	for(;;)
    {
        ...
        if(view.empty())          // If there are no more images stop the loop
        {
            ...
            {
                std::cout << "detected board num is " << imagePoints.size() << "(less than 3)" << std::endl;
                return -1;
            }
        ...
        {
                s.valid_imgs_lst.push_back(img_address);
        ...
        else {key = (char)waitKey(s.inputCapture.isOpened() ? 50 : s.delay);}
        if( key  == ESC_KEY )
            break;
